...
> **产品基线更新（2026-08-02）**：本文件中的产品定位、五 Tab、三记录模式、自动备忘、记忆河、用户端核心闭环和 MVP 产品范围，已由 [`内在地形-产品基线-v2.md`](内在地形-产品基线-v2.md) 替代。身份、安全、权限、证据链、删除、发布、部署和工程质量要求在不与 V2 冲突时继续有效；后续将按 V2 单独升级技术基线。
...
| Chat API 默认 `confide`；`auto` 仅兼容旧客户端且同样按 `confide` 处理；只有显式 `memo/life` 才创建对应内容 | 新客户端使用明确意图和独立手动入口，稳定后移除 `auto` |
